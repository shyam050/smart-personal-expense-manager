"""
Seed script for Smart Expense Manager — inserts 6 months of realistic
transaction data to properly test:

  1. Predictive Budget Forecasting
     → Needs 3+ months of varied per-category spending history
     → Includes deliberate trends (Food spend trending UP, Transport DOWN)
     → Includes seasonal patterns (higher spend in some months)

  2. Auto-categorization (NLP)
     → Uses real Indian merchant names from the training set
     → Covers all 8 categories the model knows

Usage:
  python seed_data.py --user-email your@email.com --password yourpassword
  python seed_data.py --user-email your@email.com --password yourpassword --host localhost --port 3306

The script:
  1. Finds (or creates) a user matching --user-email in your DB
  2. Creates the 8 standard categories if they don't exist
  3. Inserts 6 months + current month partial of transactions
...
# ─── Category definitions matching the NLP model's output labels ────────────
CATEGORIES = [
...
# ─── Merchant data per category ─────────────────────────────────────────────
# Format: (merchant_name, min_amount, max_amount, frequency_weight)
# frequency_weight controls how often this merchant appears (higher = more often)
MERCHANTS = {
    "Food": [
        ("Zomato",          250,  800,  3),
        ("Swiggy",          200,  700,  3),
    ...
    "Entertainment": [
        ("Netflix",         649,  649,  1),   # Fixed subscription
        ("Spotify",         119,  119,  1),   # Fixed subscription
        ("BookMyShow",      300, 1200,  1),
        ("PVR Cinemas",     300, 1000,  1),
        ("Amazon Prime",    299,  299,  1),   # Fixed subscription
...
# ─── Spending patterns per month (multipliers create realistic trends) ────────
# Each category has a list of 7 multipliers [Jan, Feb, Mar, Apr, May, Jun, Jul_partial]
# Values > 1.0 = heavier spending that month, < 1.0 = lighter
# Food: trending UP over 6 months (realistic: eating out more as income grows)
# Transport: trending DOWN (got a bike/WFH)
# Entertainment: stable with a dip in exam months
# Shopping: spikes in festival months and sales
MONTHLY_MULTIPLIERS = {
